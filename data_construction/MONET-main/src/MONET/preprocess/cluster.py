...
def run_kmeans(features, n_clusters_upper=20, n_clusters_lower=5):
    ...
    for label_upper in kmeans_label_upper.unique():
        idx = kmeans_label_upper == label_upper
        if sum(idx) < n_clusters_lower:
            kmeans_label_lower[idx] = [f"{label_upper}_00" for _ in range(sum(idx))]
            kmeans_lower_dict[label_upper] = None
        else:
            kmeans_lower = KMeans(n_clusters=n_clusters_lower, random_state=42, n_init="auto").fit(
                features[idx]
            )
            kmeans_label_lower[idx] = [f"{label_upper:s}_{i:02d}" for i in kmeans_lower.labels_]
            kmeans_lower_dict[label_upper] = kmeans_lower
    print(f"[DEBUG] kmeans_label_upper unique: {kmeans_label_upper.unique().tolist()}")
    print(f"[DEBUG] kmeans_label_lower unique: {kmeans_label_lower.unique().tolist()}")
    return kmeans_label_upper, kmeans_label_lower, kmeans_upper, kmeans_lower_dict
...
if __name__ == "__main__":
    ...
    featurized = torch.load(featurized_file, weights_only=False)
    ...
    if use_pca:
        ...
        print(f"[DEBUG] n_samples={n_samples}, n_features={n_features}, n_components={n_components}")
    ...
    kmeans_label_upper, kmeans_label_lower, kmeans_upper, kmeans_lower_dict = run_kmeans(
        features=feature,
