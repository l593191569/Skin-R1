...
class Qwen2VLGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        max_pixels: Optional[int] = 448*448,
        ...
        if peft_config is not None:
            print(f"if peft_config is not None has been executed")
            model = get_peft_model(model, peft_config)
        else:
            print(f"if peft_config is None has been executed")
        ...
        if pad_token_id is None:
            ...
            # If pad token is missing, fall back to eos token
            if eos_id is not None and getattr(tok, "eos_token", None) is not None and getattr(tok, "pad_token", None) is None:
                tok.pad_token = tok.eos_token
                pad_token_id = tok.pad_token_id
        processing_class.pad_token_id = pad_token_id
        processing_class.eos_token_id = getattr(tok, "eos_token_id", None)
